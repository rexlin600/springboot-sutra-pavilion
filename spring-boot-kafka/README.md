...
## kafka 的 Java 消费者是如何管理TCP连接的

> 需要明确 Java 消费者什么时候创建连接、关闭连接

- 和生产者不同的是，构建 `KafkaConsumer` 实例时是不会创建任何 `TCP` 连接的
- 创建连接的时机：`TCP` 连接是在调用 `KafkaConsumer.poll` 方法时被创建的（严格意义上说有三个时候可以创建 TCP 连接，如下：）
    - 发起 `FindCoordinator` 请求时（确定协调者、获取集群元数据）
    - 连接协调者时（连接协调者、令其执行组成员管理操作）
    - 消费数据时（执行实际的消息获取）
- 消费者关闭 `Socket` 也分为主动关闭和 `Kafka` 自动关闭；主动关闭是指你显式地调用消费者 `API` 的方法去关闭消费者，具体方式就是手动调用 `KafkaConsumer.close()` 方法，或者是执行 `Kill` 命令
...
**请注意分区、Consumer中的Offset的含义：**

- 消息在分区中的位移 Offset 代表的是消息在分区中保存的位置
- 另一个 Offset 指的是在 Consumer 端，它指的是记录要消费的下一条消息的位移
...
- 手动提交：由 Consumer 手动提交位移（手动提交还有同步提交、异步提交的区别）
    - 同步提交
    - 异步提交
...
## Kafka CommitFailedException异常怎么处理？

> 这种异常出现在 `Consumer` 端在提交位移时发生了错误或异常，而且还是那种不可恢复的严重异常！

官方给出的发生这种异常的原因是：`你的消费者实例连续两次调用 poll 方法的时间间隔超过了期望的 max.poll.interval.ms 参数值。这通常表明，你的消费者实例花费了太长的时间进行消息处理，耽误了调用 poll 方法`
...
令人遗憾的是，目前官方没有针对这种异常有任何描述、说明，并且前面提到的四种方法是无法处理这种异常的。因此这里给出一个建议：

- 比起返回 `CommitFailedException` 异常只是表明提交位移失败，更好的做法应该是，在 `Consumer` 端应用程序的某个地方，能够以日志或其他方式友善地提示你错误的原因，这样你才能正确处理甚至是预防该异常


## Kafka 多线程开发消费实例

> 在 `Kafka 0.10.1.0` 之后，更改为了双线程，即用户主线程、心跳线程；老版本 Consumer 是多线程的架构，每个 Consumer 实例在内部为所有订阅的主题分区创建对应的消息获取线程，也称 Fetcher 线程。老版本 Consumer 同时也是阻塞式的（blocking），Consumer 实例启动后，内部会创建很多阻塞式的消息获取迭代器

- `用户主线程`：就是你启动 `Consumer` 应用程序 `main` 方法的那个线程
- `心跳线程`：新引入的心跳线程（Heartbeat Thread）只负责定期给对应的 `Broker` 机器发送心跳请求，以标识消费者应用的存活性（liveness）；此外还有一个目的就是它能将心跳频率与主线程调用 `KafkaConsumer.poll` 方法的频率分开，从而解耦真实的消息处理逻辑与消费者组成员存活性管理

在此之前需要明确：

- `KafkaConsumer` 类不是线程安全的，简单来说就是你不能在多线程中共享一个 `KafkaConsumer` 实例，否则程序会抛出 `ConcurrentModificationException` 异常
- 另外 `KafkaConsumer` 中有一个方式是例外的，它就是 `wakeup()`，可以在其他线程汇总安全地调用 `KafkaConsumer.wakeup()` 来唤醒 `Consumer`

**多线程方案：**

- 消费者程序启动多个线程，每个线程维护专属的 `KafkaConsumer` 实例，负责完整的消息获取、消息处理流程
  - ![](https://static001.geekbang.org/resource/image/25/2c/25c8e38237117c57047997ecba5dd52c.png)
- 消费者程序使用单或多线程获取消息，同时创建多个消费线程（专门的线程池）执行消息处理逻辑（实现消息获取、消息处理的解耦）
  - ![](https://static001.geekbang.org/resource/image/de/c1/deb0d00a5ede97f270cf42a255287fc1.png)

(图片摘自 GeekTime 专栏：Kafka核心技术与实战)

| 方案 | 优点 | 缺点 |
| ---- | ---- | ---- |
| 多线程 + 多 KafkaConsumer 实例 | 方便实现 </br> 速度快 </br> 无线程间开销、易于维护分区内的消费顺序 | 占用更多的系统资源 </br> 线程数受限于主题分区数、扩展性差 </br> 线程自己处理消息超时从而引发 Rebalance |
| 单线程 + 单 KafkaConsumer 实例 + 消息处理Worker线程池 | 可独立扩展消费获取线程数和Worker线程数 </br> 伸缩性好 | 实现难度高 </br> 难以维护分区内的消息消费顺序 </br> 处理链路拉长且不易位移提交管理 |
